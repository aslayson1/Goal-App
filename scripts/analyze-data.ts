...
const sampleData = [
  { name: "Goal 1", progress: 75, category: "Health" },
  { name: "Goal 2", progress: 50, category: "Career" },
  { name: "Goal 3", progress: 90, category: "Personal" },
]

console.log("📈 Data Analysis Results:")
console.log("Total goals:", sampleData.length)
console.log("Average progress:", sampleData.reduce((sum, goal) => sum + goal.progress, 0) / sampleData.length)
console.log("Categories:", [...new Set(sampleData.map((goal) => goal.category))])

// Group by category
const byCategory = sampleData.reduce(
  (acc, goal) => {
...
console.log("Goals by category:", byCategory)
...
function analyzeGoalsByCategory(goals: GoalData[]) {
  ...
  Object.keys(categoryData).forEach((category) => {
    categoryData[category].avgProgress = Math.round(categoryData[category].totalProgress / categoryData[category].count)
  })
...
function analyzeGoalsByPriority(goals: GoalData[]) {
  const priorityData = goals.reduce(
    (acc, goal) => {
