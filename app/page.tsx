...
function GoalTrackerApp() {
  ...
  async function loadCategoriesAndGoalsFromDB(userId: string) {
    try {
      ...
      for (const category of categories) {
        ...
        goalsData[category.name] = goals.map((goal) => ({
          id: goal.id,
          title: goal.title,
          description: goal.description,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <h1 className="text-2xl font-bold text-gray-900">Hi {user?.user_metadata?.full_name || "Scott"},</h1>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button onClick={() => setShowProfile(true)} variant="ghost" size="sm" className="p-0">
              <Avatar className="h-8 w-8">
                <AvatarImage src={user?.user_metadata?.avatar_url || "/placeholder.svg"} />
                <AvatarFallback className="bg-[#05a7b0] text-white text-sm">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <Target className="h-8 w-8 text-[#05a7b0]" />
              </div>
            </CardContent>
          </Card>
          <Card>
                          ...
                          return (
                            <div key={goal.id} className="space-y-3">
                              ...
                              <div className="flex items-center justify-between text-sm">
                                ...
                                <span
                                  className={`font-medium ${weeklyProgress.onTrack ? "text-green-600" : "text-red-600"}`}
                                >
                              ...
                              {goalType === "binary" ? (
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Button
                                    ...
                                    className="ml-auto"
                                  >
                                    {goal.currentCount >= goal.targetCount ? "✓" : "Complete"}
                                  </Button>
                                </div>
                              )}
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="flex items-center space-x-2">
                      <Calendar className="h-5 w-5" />
                      <span>Week {currentWeek} Tasks</span>
                    </CardTitle>
                  ...
                  <div className="space-y-3">
                    {weeklyTasks[`Week ${currentWeek}`] && weeklyTasks[`Week ${currentWeek}`].length > 0 ? (
                      <DndContext
                        sensors={sensors}
                        collisionDetection={closestCenter}
                        onDragEnd={(event) => handleWeeklyTaskDragEnd(event, `Week ${currentWeek}`)}
                      >
                        <SortableContext
      ...
      {/* All the existing dialogs and modals remain the same */}
      {/* Add Goal Dialog */}
