...
function GoalTrackerApp() {
  ...
  const loadCategoriesAndGoalsFromDB = async (userId: string) => {
    try {
      ...
      goals?.forEach((goal) => {
        ...
        if (categoryName && goalsData[categoryName]) {
          goalsData[categoryName].push({
            id: goal.id,
            title: goal.title,
  ...
  const loadTasksFromDB = async (userId: string) => {
    try {
      ...
      const dailyTasksData: Record<string, DailyTask[]> = {}

      weeklyTasks?.forEach((task) => {
        const weekKey = `Week ${task.week_number}`
        if (!weeklyTasksData[weekKey]) {
          weeklyTasksData[weekKey] = []
        }
        weeklyTasksData[weekKey].push({
          id: task.id,
          title: task.title,
      ...
      dailyTasks?.forEach((task) => {
        const dayKey = task.day_of_week
        if (!dailyTasksData[dayKey]) {
          dailyTasksData[dayKey] = []
        }
        dailyTasksData[dayKey].push({
          id: task.id,
          title: task.title,
          description: task.description || "",
          category: task.category || "",
      ...
      return { weeklyTasks: weeklyTasksData, dailyTasks: dailyTasksData }
    } catch (error) {
      console.error("Error loading tasks from database:", error)
      return { weeklyTasks: initialWeeklyTasks, dailyTasks: initialDailyTasks }
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto p-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Goal Tracker</h1>
          ...
          {["daily", "weekly", "12-week", "1-year", "5-year"].map((view) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                activeView === view ? "bg-blue-600 text-white" : "bg-white text-gray-700 hover:bg-gray-100"
              }`}
            >
              {view === "12-week"
                ? "12-Week Goals"
        ...
        {activeView === "daily" && (
          <div>
            ...
            <p className="text-gray-600">Daily task management coming soon...</p>
          </div>
        )}
        ...
        {activeView === "weekly" && (
          <div>
            ...
            <p className="text-gray-600">Weekly task management coming soon...</p>
          </div>
        )}
        ...
        {activeView === "12-week" && (
          <div>
            ...
            <p className="text-gray-600">12-week goal management coming soon...</p>
          </div>
        )}

        {activeView === "1-year" && (
          <div>
            <h2 className="text-2xl font-semibold mb-4">1-Year Goals</h2>
            <p className="text-gray-600">1-year goal management coming soon...</p>
          </div>
        )}

        {activeView === "5-year" && (
          <div>
            <h2 className="text-2xl font-semibold mb-4">5-Year Goals</h2>
            <p className="text-gray-600">5-year goal management coming soon...</p>
          </div>
        )}
      </div>
    </div>
